              ...
              <DialogPanel class="pointer-events-auto relative w-screen max-w-md">
                ...
                <div class="flex h-full flex-col overflow-y-scroll bg-gray-800 py-6 shadow-xl">
                  <div class="px-4 sm:px-6">
                    <DialogTitle class="text-lg font-medium text-gray-200"> Invited </DialogTitle>
                  </div>
                  <div class="relative mt-6 flex-1 px-4 sm:px-6 flex-col">
										<div class="invite-wrap" v-for="list in store.getters.userInviteList" v-bind:key="list">
...
function insertRoomData(friendId: number) {
	store.getters.gameSocket.emit("joinFriend", friendId);
	closeSlider();
}

function refuse(friendId: number){
	store.getters.gameSocket.emit("refuse", friendId);
	closeSlider();
}
